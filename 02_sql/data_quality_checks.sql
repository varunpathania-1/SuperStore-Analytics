...
WHERE table_schema = DATABASE() AND table_name = 'stg_retail_sales';
...
COUNT(*) AS duplicate_count
FROM stg_retail_sales
GROUP BY order_id, customer_id, product_name, order_date
HAVING COUNT(*) > 1;
...
SELECT order_id, quantity, unit_price, discount_pct, sales_amount,
ROUND(quantity * unit_price * (1 - discount_pct / 100), 2) AS expected_sales
FROM stg_retail_sales
WHERE ABS(sales_amount - ROUND(quantity * unit_price * (1 - discount_pct / 100), 2)) > 0.01;
