...
SELECT
ROUND(SUM(shipping_cost) * 100.0 / SUM(sales_amount),2) AS shipping_cost_pct
FROM retail_sales_cleaned
HAVING SUM(sales_amount) > 0;

-- Orders with highest shipping cost
SELECT order_id, customer_name, shipping_cost, sales_amount, profit
FROM retail_sales_cleaned
...
ROUND(SUM(shipping_cost),2) AS total_shipping_cost,
ROUND(SUM(profit),2) AS total_profit,
ROUND(SUM(shipping_cost) * 100.0 / SUM(profit),2) AS shipping_to_profit_pct
FROM retail_sales_cleaned
GROUP BY product_category
HAVING SUM(profit) > 0
...
SELECT order_id, customer_name, product_name, sales_amount, discount_pct, profit
FROM retail_sales_cleaned
...
SELECT product_name, SUM(sales_amount) total_sales,
...
ROUND(SUM(SUM(sales_amount)) OVER(ORDER BY order_date),2) AS running_sales
FROM retail_sales_cleaned
GROUP BY order_date
ORDER BY order_date;
...
OVER(ORDER BY order_date),2) AS running_profit
FROM retail_sales_cleaned
GROUP BY order_date;

-- Previous month sales
WITH monthly_sales AS (
SELECT YEAR(order_date) year, MONTH(order_date) month, SUM(sales_amount) sales
FROM retail_sales_cleaned
GROUP BY YEAR(order_date),MONTH(order_date)
)
SELECT year, month, sales,
LAG(sales) OVER(ORDER BY year,month) previous_month_sales
FROM monthly_sales;
...
SELECT year, month, sales,
ROUND((sales - LAG(sales) OVER(ORDER BY year,month)) / LAG(sales) OVER(ORDER BY year,month)*100,2) AS mom_growth
FROM monthly_sales;
...
SELECT year, month, sales,
ROUND(AVG(sales) OVER(ORDER BY year,month ROWS BETWEEN 2 PRECEDING AND CURRENT ROW),2) AS rolling_3_month_avg
FROM monthly_sales;

-- Top products by category
WITH ranked_products AS (
SELECT product_category, product_name, SUM(sales_amount) sales,
...
SELECT region, customer_name, SUM(profit) profit,
ROW_NUMBER() OVER(PARTITION BY region ORDER BY SUM(profit) DESC
) rn
FROM retail_sales_cleaned
GROUP BYregion,customer_name
)
SELECT *
...
-- Quartile analysis
SELECTcustomer_name, SUM(sales_amount) total_sales,
NTILE(4) OVER(ORDER BY SUM(sales_amount) DESC) AS sales_quartile
FROM retail_sales_cleaned
GROUP BY customer_name;

-- First order value per customer
SELECT customer_name, order_date, sales_amount,
FIRST_VALUE(sales_amount) OVER(PARTITION BY customer_name ORDER BY order_date) AS first_order_sales
FROM retail_sales_cleaned;

-- Latest order value
SELECT customer_name, order_date, sales_amount,
LAST_VALUE(sales_amount) OVER(PARTITION BY customer_name ORDER BY order_date ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS latest_order_sales
FROM retail_sales_cleaned;

-- Cusotmer purchase sequence
SELECT customer_name, order_date, sales_amount,
ROW_NUMBER() OVER(PARTITION BY customer_name ORDER BY order_date) AS purchase_number
FROM retail_sales_cleaned;
...
SUM(sales_amount) total_sales,
ROW_NUMBER() OVER(PARTITION BY product_category ORDER BY SUM(sales_amount) DESC
) rn
FROM retail_sales_cleaned
GROUP BY product_category, product_name
)
...
SELECT rs.region,rs.total_sales,
ROUND(rs.total_sales / cs.grand_total * 100,2) AS contribution_pct
...
SELECT product_category, product_name,
SUM(profit) total_profit,
ROW_NUMBER() OVER(PARTITION BY product_category ORDER BY SUM(profit) DESC
) rn
FROM retail_sales_cleaned
GROUP BY product_category, product_name
)
...
SUM(sales_amount) total_sales,
SUM(profit) total_profit,
COUNT(DISTINCT order_id) as total_orders,SUM(quantity) total_quantity
FROM retail_sales_cleaned
GROUP BY YEAR(order_date), MONTH(order_date)
)
SELECT year, month,
ROUND(total_sales,2) total_sales,
ROUND(total_profit,2) total_profit, total_orders,total_quantity,
...
FROM
(
SELECT SUM(sales_amount) AS order_sales
FROM retail_sales_cleaned
GROUP BY order_id
) avg_orders
);
...
WHERE r.product_name = p.product_name AND r.return_flag = 'Yes'
)
...
SELECT customer_id, customer_name, order_id, order_sales
FROM
(
SELECT customer_id, customer_name, order_id,
SUM(sales_amount) AS order_sales,
ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY SUM(sales_amount) DESC
) AS rn
FROM retail_sales_cleaned
GROUP BY customer_id, customer_name, order_id) ranked_orders
WHERE rn = 1
